...
arma::mat calculate_RE_onepatch(Rcpp::List x, Rcpp::NumericVector param, 
                                int secondInclude ){
        
  double theta_P = param["theta_P"];
  double theta_M = param["theta_M"];
  double theta_H = param["theta_H"];
  double gamma = param["gamma"];
  double mu_H = param["mu_H"];
  double f_P = param["f_P"];
  double f_M =param["f_M"];
  double c_MP = param["c_MP"];
  double c_PM = param["c_PM"];
  double c_MM = param["c_MM"];
  double c_PP = param["c_PP"];
  double ntime = param["ntime"];
  ...
  for (int j = 0; j <= ntime - 100; ++j){
    ...
    V_mat(1,2) = p_out_nondiag;
    V_mat(2,2) = m_out_diag;
    V_mat(2,1) = m_out_nondiag; //in R (3,3)
