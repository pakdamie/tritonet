...
Rcpp::List discrete_trito_model_rcpp_ONEPATCH(
  ...
  for (int j = 0; j < ntime; j++) {
    ...
    arma::rowvec PS_Rates = (b_P * NP) - (infections_P + c_MP * NM + c_PP * NP) % PS.row(j);
    arma::rowvec PI_Rates = (infections_P % PS.row(j)) - (c_MP * NM + c_PP * NP) % PI.row(j);
    
    // Secondary vector rates
    arma::rowvec MS_Rates = (b_M * NM) - (infections_M + c_PM * NP + c_MM * NM) % MS.row(j);
    arma::rowvec MI_Rates = (infections_M % MS.row(j)) - (c_PM * NP + c_MM * NM) % MI.row(j);
