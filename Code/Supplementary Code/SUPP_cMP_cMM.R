...
c_MP_standard <- param_standard["c_MP"] ## Competition effect of p.vector on s.vector
...
competition_param_list <- vary_parameter_value(
        param_standard, c("c_MM", "c_MP", "mortality_P"), competition_param
)
...
RE_COMPETITION <-
        Simulate_Model_Output(
                ...
                infection_start = "No",
                variable_interest = c("c_MM", "c_MP", "mortality_P"),
...
ggplot(RE_COMPETITION, aes(
        ...
        xlab(expression("Modifier of secondary on primary competition ("*c[PM]*")")) +
