...
discrete_trito_model <- function(ntime, 
  ...
  for (j in 1:(ntime - 1)) {
      ...
      dispersal_PI <-  rowSums(sweep(adjacency_matrix_adj, MARGIN =1, (- d * PI_mat[j, ]),`*`)) + 
        t(adjacency_matrix_adj) %*% (d * PI_mat[j, ]) 
      
      dispersal_SS <- rowSums(sweep(adjacency_matrix_adj, MARGIN =1, (- d * SS_mat[j, ]),`*`)) + 
        t(adjacency_matrix_adj) %*% (d * SS_mat[j, ]) 
      
      dispersal_SI <- rowSums(sweep(adjacency_matrix_adj, MARGIN =1, (- d * SI_mat[j, ]),`*`)) + 
        t(adjacency_matrix_adj) %*% (d * SI_mat[j, ]) 
