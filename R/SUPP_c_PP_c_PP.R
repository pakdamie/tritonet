# Vary the competition coefficient of M on P as well as M on M.

param_standard <- get_parameters("standard")
c_PM_standard <- param_standard["c_PM"] ## Competition effect of p.vector on s.vector
c_PP_standard <- param_standard["c_PP"] ## Competition effect of s.vector on s.vector
...
competition_param_list <- vary_parameter_value(
        param_standard, c("c_PP", "c_PM", "mortality_P"), competition_param
)
...
RE_COMPETITION <-
        Simulate_Model_Output(
                ...
                infection_start = "No",
                variable_interest = c("c_PP", "c_PM", "mortality_P"),
...
ggplot(RE_COMPETITION, aes(
        ...
        ylab(expression("Modifier of primary on primary competition ("*c[PP]*")")) +
...
ggsave(here("Figures", "RE_DF_cmm_cmp_R0.pdf"),
...
ggplot(RE_COMPETITION, aes(
        x = c_PM / c_PM_standard,
        y = c_PP / c_PP_standard, fill = max_NM
)) +
        ...
        scale_x_continuous(expand = c(0,0)) + 
        scale_y_continuous(expand = c(0,0)) + 
        xlab(expression("Modifier of secondary on primary competition ("*c[MP]*")")) +
